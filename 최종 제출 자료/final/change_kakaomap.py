...
def address_to_latitude(address):
    ...
    result = requests.get(urlparse(url).geturl(),headers={'Authorization': 'KakaoAK {}'.format('012f3277daabe10113b230330bd12a77')})
    ...
    while (perpect == False):
        ...
        for i in range(count):
            ...
            for j in loc_df[loc_df['label'] == i]['latitude']:
                if str(j) in df1['lat'].unique():
                    break
                else:
                    chk += 1
                if chk == len(loc_df[loc_df['label'] == i]):
                    if chk>100 or chk < 20:  # 10개보다 작은 경우 외곽 지역에서 작은 군집이 추출되는거라
                        break
                    perpect = True
